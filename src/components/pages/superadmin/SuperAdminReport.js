...
const SuperAdminReport = () => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://192.168.100.99:8000/components/superAdminReport.php?action=get_all_reports');
  ...
  const handleUpdateStatus = async (reportId, newStatus) => {
    try {
      const response = await fetch('http://192.168.100.99:8000/components/superAdminReport.php', {
        method: 'PUT',
        headers: {
          ...
          <div className="bg-gray-500 p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              ...
              <div className="flex flex-wrap gap-2 mt-4 md:mt-0">
                <button 
                  onClick={() => navigate('/gomez/super-admin')}
