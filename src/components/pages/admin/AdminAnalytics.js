...
const AdminAnalytics = () => {
  ...
  useEffect(() => {
    const fetchServiceTypes = async () => {
      try {
        const response = await fetch('http://192.168.100.99:8000/components/analytics.php?action=service-types');
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.100.99:8000/components/analytics.php?${queryParams}`);
      const result = await response.json();
