...
export const useDocuments = (userData) => {
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${n}/api/components/documents.php?user_id=${userData.id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
  ...
  const handleDocumentUpload = async (file, documentDetails) => {
    ...
    const response = await fetch(`${n}/api/components/documents.php`, {
      method: 'POST',
      body: formData
    });
    return await response.json();
...
export const useBugReport = (userData, email) => {
  ...
  const handleReportBug = async (e) => {
    ...
    try {
      const response = await fetch(`${n}/api/components/report.php`, {
        method: 'POST',
